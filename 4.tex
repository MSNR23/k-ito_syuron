...
\subsection{報酬}
報酬は，リリース時の投射角度$\theta_{\mathrm{v}}$と手先速度$v_{2}$の2つの要素から設定した．リリースの投射角度$\theta_{\mathrm{v}}$について，\equref{2.19}で計算した．
\begin{eqnarray}
  \equlabel{2.19}
  \theta_{\mathrm{v}}=\theta_{1}+\theta_{2}-90
\end{eqnarray}
\equref{2.19}において，$\theta_{\mathrm{v}}$，$\theta_{1}$，$\theta_{2}$の単位はdegである．
また，条件は以下の通りとした．\\
\begin{itemize}
